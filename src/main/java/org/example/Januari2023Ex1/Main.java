package org.example.Januari2023Ex1;
...
public class Main {
    ...
    public static void PodeliListaProsek(DLL<Integer> list, int n){
        ...
        prev=null;
        DLL<Integer> smallerThenAvg=new DLL<>();
        DLL<Integer> biggerThenAvg=new DLL<>();
        while (current!=null){
            if(current.element<=avg){
                smallerThenAvg.insertLast(current.element);
            }else {
               biggerThenAvg.insertLast(current.element);
            }
            current=current.pred;
        }
        System.out.println(smallerThenAvg);
